...
async def chat(
    ...
    try:
        reply = await run_in_threadpool(app_services.agent.reply, payload.message.strip(), current_session)
...
def reset(request: Request, app_services: ApplicationServices = Depends(services)):
